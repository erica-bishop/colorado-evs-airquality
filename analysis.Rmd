...
Run a time series decomposition with both?

```{r}

# ev_oz_ts <- left_join(ev_ts, ozone_ts, by = "my_date")
# 
# ev_oz_decomp <- ev_oz_ts |> 
#   model(
#     classical_decomposition(c(ev_reg_count, monthly_ozone_avg_ppm), type = "additive")
#   ) |> 
#   components() |> 
#   autoplot() +
#   labs(title = "Additive Decomp of EV registrations and ozone in CO")
# 
# 
...
cor_table <- data.frame("County" = c("El Paso", "Denver", "Arapahoe", "Jefferson", "Adams"),
                        "Correlation" = c(round(elpaso_cor, 4), round(denver_cor, 4), round(arapahoe_cor, 4), round(jeff_cor, 4), round(adams_cor, 4)))

cor_gt <- gt(cor_table) |> 
  tab_header("Pearson Correlation Coefficients")

gtsave(cor_gt, "county_ev_cor_table.png", path = datadir)
...
my_summary |> 
  ggplot(aes(x = my_date,
             ...
             color = ev_reg_count)) +
  geom_point() +
  geom_line(data = augment(intmod),
            aes(y = .fitted,
                color = ev_reg_count))


# intmodplot <- ggplot(data = my_summary,
#                      aes(x = my_date,
#                          y = monthly_ozone_avg_ppm,
#                          color = ev_reg_count)) +
